package com.lenois.kotliniscool


//Given an array of integers, find the pair of adjacent elements
//that has the largest product and return that product.
fun adjacentElementsProduct(input: List<Int>): Int {
    return 0
}
//Below we will define an n-interesting polygon. Your task is to find the area of a polygon for a given name.
//A 1-interesting polygon is just a square with a side of length 1. An n-interesting polygon is obtained by
//taking the n - 1-interesting polygon and appending 1-interesting polygons to its rim, side by side.
// You can see the 1-, 2-, 3- and 4-interesting polygons in the picture below.
fun interestingPolygon(n: Int): Int{
    return 0
}
//given a list of ints intList find the minimum number of number of ints needed to fill the list
//with ever int in range min(l) to max(l)
fun makeArrayConsecutive(intList: List<Int>): Int {
    return 0
}
